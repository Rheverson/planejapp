    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
      <style>{`
        ...
        * { -webkit-tap-highlight-color: transparent; }
        ...
        .finn-glow { box-shadow: 0 0 20px rgba(139, 92, 246, 0.5), 0 4px 24px rgba(99, 102, 241, 0.4); }
        .finn-glow-active { box-shadow: 0 0 30px rgba(139, 92, 246, 0.7), 0 4px 32px rgba(99, 102, 241, 0.6); }
      `}</style>

      {showProfileIcon && (
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
          className="fixed z-40" style={{ top: '44px', right: '16px' }}>
      ...
      <main className="dark:bg-gray-900" style={{
      ...
      <motion.nav initial={{ y: 100 }} animate={{ y: 0 }}
        ...
        <div className="max-w-lg mx-auto relative" style={{
          display: 'grid',
            ...
            return (
              <Link key={item.page} to={createPageUrl(item.page)}
                ...
                <div className={`p-1.5 rounded-xl transition-all duration-300 ${isActive ? 'bg-blue-50 dark:bg-blue-900/40' : ''}`}>
                  <item.icon className={`w-5 h-5 transition-colors duration-300 ${isActive ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500'}`} />
                </div>
                <span className={`text-[10px] mt-0.5 transition-all duration-300 ${isActive ? 'text-blue-600 dark:text-blue-400 font-bold' : 'text-gray-400 dark:text-gray-500'}`}>{item.name}</span>
              </Link>
            );
          })}

          <div className="flex flex-col items-center justify-end pb-1 relative">
            <span className={`text-[10px] font-bold mb-0.5 transition-colors duration-300 ${isAIActive ? 'text-violet-600 dark:text-violet-400' : 'text-gray-500 dark:text-gray-400'}`}>
              Finn
            </span>
            <motion.button whileTap={{ scale: 0.92 }} whileHover={{ scale: 1.05 }}
              onClick={() => navigate(createPageUrl('AIInsights'))}
              className={`absolute bottom-6 w-14 h-14 rounded-2xl flex items-center justify-center ${isAIActive ? 'finn-glow-active' : 'finn-glow'} bg-gradient-to-br from-violet-500 via-purple-600 to-indigo-600`}>
              {isAIActive && (
                <motion.div className="absolute inset-0 rounded-2xl bg-violet-400"
                  animate={{ scale: [1, 1.15, 1], opacity: [0.4, 0, 0.4] }}
                  transition={{ duration: 2, repeat: Infinity }} />
              )}
              <motion.div animate={{ rotate: isAIActive ? [0, 10, -10, 0] : 0 }}
