  ...
  const confirmAction = async () => {
    ...
    try {
      if (action._type === "tx" && !action.intent) {
        ...
        const { error } = await supabase.from("transactions").insert({ user_id: user.id, type: action.type, amount: action.amount, description: action.description, category: action.category, account_id: accountId, date: action.date || today, is_realized: action.is_realized ?? true });
        ...
        const { error } = await supabase.from("transactions").insert({ user_id: user.id, type: "transfer", amount: action.amount, description: "Transferência", account_id: action.from_account_id, transfer_account_id: action.to_account_id, date: action.date, is_realized: true });
        if (error) throw error;
        setPendingAction(null);
        ...
        await supabase.from("transactions").insert({ user_id: user.id, type: "expense", amount: action.paid_amount, description: action.description, category: action.category, account_id: accountId, date: action.date || today, is_realized: true });
        setPendingAction(null);
        ...
        const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/send-email`, {
          ...
          body: JSON.stringify({ to: action.email, senderEmail: "noreply@planejapp.com.br", senderName: "Planeje", subject: `Você foi convidado para o Planeje! 💜`, html: `<p>Olá${action.name ? ", " + action.name : ""}! Acesse: <a href="${referralLink}">${referralLink}</a></p>` })
        });
...
function AnalysisTab({ user }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
      {usage && (
        <div className="bg-violet-50 dark:bg-violet-900/20 rounded-xl px-3 py-2 border border-violet-200 dark:border-violet-800 flex items-center justify-between">
          <div className="flex items-center gap-2"><Clock className="w-3.5 h-3.5 text-violet-600" /><p className="text-xs text-violet-700 dark:text-violet-300">Análises restantes esta semana</p></div>
          <span className="font-bold text-sm text-violet-600">{usage.remaining}/2</span>
        </div>
      )}
      ...
      {limiteAtingido && (
        <div className="bg-amber-50 dark:bg-amber-900/20 rounded-2xl p-5 border border-amber-200 dark:border-amber-800 text-center">
          ...
          <p className="text-amber-800 dark:text-amber-300 font-medium mb-1 text-sm">Limite semanal atingido</p>
          <p className="text-amber-600 dark:text-amber-400 text-xs">{error}</p>
        </div>
      )}
        ...
        {showPeriodSelector && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3 flex items-center gap-2">
              <Calendar className="w-4 h-4 text-violet-600" /> Qual período analisar?
            </h3>
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div>
                <p className="text-xs font-medium text-gray-400 mb-1">Mês</p>
              ...
              <div>
                <p className="text-xs font-medium text-gray-400 mb-1">Ano</p>
            ...
            <div className="bg-violet-50 dark:bg-violet-900/20 rounded-xl p-2.5 mb-3 text-center">
              <p className="text-xs text-violet-700 dark:text-violet-300 font-medium capitalize">📊 {monthLabel}</p>
            </div>
            <div className="flex gap-2">
              <Button onClick={() => setShowPeriodSelector(false)} variant="outline" className="flex-1 h-9 rounded-xl text-sm">Cancelar</Button>
              <Button onClick={fetchInsights} className="flex-1 h-9 bg-gradient-to-r from-violet-600 to-indigo-600 rounded-xl text-sm text-white">
      ...
      {!data && !loading && !limiteAtingido && !showPeriodSelector && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <h3 className="text-base font-medium text-gray-900 dark:text-white mb-1">Análise Financeira</h3>
          <p className="text-gray-400 text-xs mb-1">Score de saúde, insights e recomendações.</p>
          <p className="text-xs text-gray-400 mb-5">📊 2 análises gratuitas por semana</p>
          <Button onClick={() => setShowPeriodSelector(true)} className="w-full h-11 bg-gradient-to-r from-violet-600 to-indigo-600 rounded-xl font-medium text-white">
      ...
      {loading && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-8 border border-gray-100 dark:border-gray-700 text-center">
          <motion.div animate={{ rotate: 360 }} transition={{ duration: 2, repeat: Infinity, ease: "linear" }} className="w-12 h-12 bg-violet-100 dark:bg-violet-900/30 rounded-2xl flex items-center justify-center mx-auto mb-3">
            <Sparkles className="w-6 h-6 text-violet-600" />
          </motion.div>
          <p className="text-gray-700 dark:text-gray-300 font-medium text-sm mb-1">Analisando suas finanças...</p>
      ...
      {data && !loading && (<>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          className={`bg-gradient-to-br ${scoreColors[data.insights?.score_color] || scoreColors.blue} rounded-2xl p-5 text-white`}>
          <div className="flex items-center justify-between mb-3">
            <div>
              <p className="text-white/75 text-xs mb-0.5">Saúde Financeira</p>
              <p className="text-3xl font-bold">{data.insights?.score}<span className="text-lg">/100</span></p>
          ...
          {data.insights?.alerta_projecao && data.insights.alerta_projecao !== "null" && (
            <div className="bg-red-500/30 border border-red-300/50 rounded-xl p-2.5 mb-3">
              <p className="text-white text-xs font-medium">{data.insights.alerta_projecao}</p>
            </div>
          )}
        ...
        {data.insights?.recomendacoes?.length > 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden">
            <div className="px-4 pt-3 pb-1.5"><h3 className="text-xs font-medium text-gray-500 dark:text-gray-400">✂️ Onde reduzir custos</h3></div>
            <div className="divide-y divide-gray-50 dark:divide-gray-700">
              {data.insights.recomendacoes.map((rec, i) => (
                <div key={i} className="px-4 py-3">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-sm font-medium text-gray-800 dark:text-gray-200 capitalize">{rec.categoria}</p>
                    <span className="text-xs font-medium text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 px-2 py-0.5 rounded-full">Economize {fmt(rec.economia_possivel)}</span>
                  </div>
                  ...
                  <p className="text-xs text-gray-400">{rec.dica}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {data.insights?.investimento_sugerido && (
          <div className={`rounded-2xl p-4 text-white ${data.meta?.isProjectedNegative ? "bg-gradient-to-br from-red-500 to-rose-600" : "bg-gradient-to-br from-emerald-500 to-teal-600"}`}>
            ...
            <p className="text-2xl font-bold">{fmt(data.insights.investimento_sugerido.valor)}</p>
        ...
        <div className="flex gap-2 pb-2">
          <Button onClick={() => setShowPeriodSelector(true)} variant="outline" className="flex-1 h-10 rounded-xl border-gray-200 text-gray-600 text-sm">
            <RefreshCw className="w-3.5 h-3.5 mr-1.5" /> Nova análise
          </Button>
          <button onClick={deleteInsights} className="h-10 px-4 rounded-xl border border-red-200 text-red-500 text-sm hover:bg-red-50 transition-colors">
            Apagar
          </button>
        </div>
      </>)}
...
export default function AIInsights() {
  ...
  return (
    <div className="flex flex-col bg-gray-50 dark:bg-gray-900"
      style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: `${NAV_HEIGHT}px` }}>

      {/* ── HEADER COMPACTO ─────────────────────────────────── */}
      <div className="bg-gradient-to-br from-violet-700 via-purple-800 to-indigo-900 px-5 flex-shrink-0"
        style={{ paddingTop: "max(44px, calc(env(safe-area-inset-top, 0px) + 10px))", paddingBottom: 0 }}>

        {/* Finn avatar + nome — compacto */}
        <div className="flex items-center gap-3 mb-4">
          <div className="relative">
            <motion.div
              ...
              className="w-11 h-11 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center border border-white/30"
            >
          ...
          <div>
            ...
            <div className="flex items-center gap-1">
              ...
              <p className="text-violet-200 text-xs">Consultor financeiro IA</p>
            </div>
          </div>
        </div>
