...
function AnalysisTab({ user }) {
  ...
  const loadSavedInsights = async () => {
    ...
    try {
      ...
      if (profile?.ai_insights) { setData(profile.ai_insights); setSavedDate(profile.ai_insights_date); }
    } catch (err) { console.error(err); }
    finally { setLoadingSaved(false); }
  };

  const saveInsights = async (insightsData) => {
    await supabase.from('profiles').update({ ai_insights: insightsData, ai_insights_date: new Date().toISOString() }).eq('id', user.id);
  };

  const deleteInsights = async () => {
    await supabase.from('profiles').update({ ai_insights: null, ai_insights_date: null }).eq('id', user.id);
    setData(null); setSavedDate(null); setUsage(null);
  };
  ...
  const fetchInsights = async () => {
    ...
    try {
      ...
      if (err) {
        try { const errBody = JSON.parse(err.context?.responseText || '{}'); if (errBody.error === 'limite_atingido') { setLimiteAtingido(true); setError(errBody.message); return; } } catch {}
        throw err;
      }
      if (result?.error === 'limite_atingido') { setLimiteAtingido(true); setError(result.message); return; }
      if (result?.error) throw new Error(result.error);
      setData(result); setUsage(result.usage); setSavedDate(new Date().toISOString());
      ...
      <div className="relative bg-gradient-to-br from-violet-600 via-purple-700 to-indigo-800 pt-12 pb-0 px-5 flex-shrink-0">
        ...
        <div className="relative flex items-center gap-4 mb-5">
          <div className="relative">
            <motion.div animate={{ boxShadow: ['0 0 0 0 rgba(167,139,250,0.4)', '0 0 0 12px rgba(167,139,250,0)', '0 0 0 0 rgba(167,139,250,0)'] }}
