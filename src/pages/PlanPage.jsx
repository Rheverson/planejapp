...
import {
  ...
  Calendar, CreditCard, ChevronRight, Sparkles, RefreshCw
} from "lucide-react";
      ...
      >
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-4 space-y-2">
            ...
            <div className="flex items-start gap-2">
              ...
              <p className="text-sm text-gray-700 dark:text-gray-300">
                Você continua com acesso <strong>Pro até {endDate}</strong>
              </p>
            </div>
            <div className="flex items-start gap-2">
              <CheckCircle2 className="w-4 h-4 text-emerald-500 flex-shrink-0 mt-0.5" />
              <p className="text-sm text-gray-700 dark:text-gray-300">
                Não há cobranças adicionais
              </p>
            </div>
            <div className="flex items-start gap-2">
              <X className="w-4 h-4 text-red-500 flex-shrink-0 mt-0.5" />
              <p className="text-sm text-gray-700 dark:text-gray-300">
                Após {endDate}, voltará ao plano gratuito
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <button
              onClick={onClose}
              className="flex-1 h-12 rounded-2xl border border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 font-medium text-sm"
            >
              Manter Pro
            </button>
            <button
...
export default function PlanPage() {
  ...
  const currentPlan = isActive ? "monthly" : "free";
  const endDate = subscription?.current_period_end
                ...
                return (
                  <div
                    ...
                    className={`bg-white dark:bg-gray-800 rounded-2xl border-2 overflow-hidden transition-all ${
                      isCurrent ? plan.border : "border-gray-100 dark:border-gray-700"
                    }`}
                  >
                    {/* Badge */}
                    {plan.badge && (
                      <div className={`bg-gradient-to-r ${plan.color} px-5 py-1.5 flex items-center justify-between`}>
                        ...
                        {isCurrent && (
                          <span className="text-white/80 text-xs">✓ Seu plano atual</span>
                        )}
                      </div>
                    )}
                    ...
                    <div className="px-5 py-4">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="text-right">
                          {plan.price > 0 ? (
                            <>
                              <p className="text-xl font-bold text-gray-900 dark:text-white">{fmt(plan.price)}</p>
                      ...
                      {isCurrent ? (
                        ...
                        <button
                          disabled
                          className="w-full h-10 rounded-xl bg-gray-100 dark:bg-gray-700 text-gray-400 text-sm font-medium"
                        >
                          Plano inferior
                        </button>
                      )}
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-600 dark:text-amber-400 mt-0.5">
                    Seu acesso Pro continua ativo até <strong>{endDate}</strong>. Após essa data, você terá acesso apenas ao plano gratuito.
                  </p>
                  <button
                    onClick={() => window.location.href = "/subscribe"}
